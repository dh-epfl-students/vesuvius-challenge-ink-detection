...
## About
This project was developed at the École Polytechnique Fédérale de Lausanne (EPFL) as part of a master's semester project. It provides a computer vision pipeline to create ink masks derived from historical papyrus fragments (sourced from the Duke University and Oslo Archives). Initially designed to overcome the 2D masking bottleneck for 3D synthetic data generation in the **Vesuvius Challenge**, this tool isolates true ink from degraded plant fibers to provide ink binary 2D mask.

## Research Summary
The extraction of clean 2D inputs from historical manuscripts is a formidable computer vision challenge. Deterministic mathematical filters (like Sauvola thresholding) often fail due to severe illumination variances and structural noise. 
...
│   ├── evaluate_loocv.py       # Leave-One-Out Cross-Validation metric calculation
...
### Training Benchmarks & Baselines
The binarization baselines and training data components incorporate the official benchmark dataset from the **ICDAR 2019 Document Image Binarization Contest (DIBCO 2019)**. The dataset is publicly downloadable via the [DIBCO 2019 Benchmark Portal](https://vc.ee.duth.gr/dibco2019/benchmark/).


> Pratikakis, I. et al. (2019). ‘ICDAR 2019 Document Image Binarization Contest (DIBCO 2019)’. In: *2019 International Conference on Document Analysis and Recognition (ICDAR)*. Sydney, Australia. URL: [https://ieeexplore.ieee.org/document/8978160](https://ieeexplore.ieee.org/document/8978160).
